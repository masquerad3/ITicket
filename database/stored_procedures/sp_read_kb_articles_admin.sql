...
CREATE OR ALTER PROCEDURE dbo.sp_read_kb_articles_admin
  @q           NVARCHAR(200) = NULL,
...
BEGIN
  ...
  WHERE (@category_id IS NULL OR a.category_id = @category_id)
    AND (@featured IS NULL OR a.is_featured = @featured)
    AND (@published IS NULL OR ISNULL(a.is_published, 1) = @published)
    AND (
      @q2 IS NULL
      OR a.title LIKE @q2
